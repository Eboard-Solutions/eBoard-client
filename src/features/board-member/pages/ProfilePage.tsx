    ...
    <MemberPortalLayout icon={Users} title="Profile & Settings" color="bg-indigo-600" subtitle="Personal information and preferences">
      <div className="flex items-center gap-1 border-b border-border/60 mb-6 overflow-x-auto">
        {(['profile', 'notifications', 'security'] as const).map((t) => (
          <button key={t} type="button" onClick={() => setTab(t)} className={`px-4 py-2.5 text-sm font-medium capitalize transition-colors border-b-2 -mb-px whitespace-nowrap ${tab === t ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-muted-foreground hover:text-foreground'}`}>
            {t}
          </button>
        ))}
      </div>
      ...
      {tab === 'profile' && (
        <div className="space-y-5">
          <div className="flex items-center gap-4 p-5 rounded-[24px] border border-border/60 bg-card shadow-sm">
            <div className="h-14 w-14 rounded-2xl bg-indigo-600 flex items-center justify-center text-white text-lg font-semibold shadow-md shadow-indigo-500/30">
              {(form.firstName?.[0] ?? '')}{(form.lastName?.[0] ?? '')}
            </div>
            <div>
              <p className="text-base sm:text-lg font-semibold tracking-tight">{form.firstName} {form.lastName}</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {[
              ...
              <div key={key} className="space-y-1.5">
                ...
                <Input value={(form as any)[key] ?? ''} onChange={(e) => setForm((p: any) => ({ ...p, [key]: e.target.value }))} className="h-9 text-sm" disabled={key === 'email'} />
              </div>
            ))}
          ...
          <Button className="w-full rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white" onClick={save} disabled={update.isPending}>
      ...
      {tab === 'notifications' && (
        <div className="space-y-4">
          ...
          {Object.entries((form.notificationPreferences ?? {}) as Record<string, boolean>).map(([key, val]) => (
            <div key={key} className="flex items-center justify-between p-4 rounded-2xl border border-border/60 bg-card shadow-sm">
              <div>
      ...
      {tab === 'security' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between p-4 rounded-2xl border border-border/60 bg-card shadow-sm">
            <div>
          ...
          <div className="p-4 rounded-2xl border border-border/60 bg-card shadow-sm space-y-3">
            <p className="text-sm font-semibold">Change Password</p>
            <Input type="password" placeholder="Current password" className="h-10 rounded-xl text-sm" />
            <Input type="password" placeholder="New password" className="h-10 rounded-xl text-sm" />
            <Input type="password" placeholder="Confirm new password" className="h-10 rounded-xl text-sm" />
            <Button size="sm" className="w-full rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white h-10" onClick={() => toast.success('Password would be updated (backend needed)')}>
              Update Password
            </Button>
          </div>

          <div className="p-4 rounded-2xl border border-amber-200 dark:border-amber-800 bg-amber-50/30 dark:bg-amber-950/10 shadow-sm">
