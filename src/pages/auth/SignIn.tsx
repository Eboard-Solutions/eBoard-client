...
import { authService } from '@/lib/auth';           // your NestJS auth service wrapper
...
export function SignIn() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Expected NestJS response shape (adjust according to your actual DTO/response)
      ...
      // Save token (your preferred storage method)
      localStorage.setItem('access_token', token);

      // Decode to get role (common pattern with NestJS + JWT)
      const decoded: any = jwtDecode(token);
      const userRole = decoded.role || response.user?.role;
      ...
      // Role-based redirect (using camelCase route keys)
      setTimeout(() => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 via-slate-100 to-blue-50/40 dark:from-slate-950 dark:via-slate-950 dark:to-indigo-950/20 px-5 py-12 sm:px-6 lg:px-8">
      ...
      {notification && (
        <div
          className={`fixed top-5 right-5 z-50 w-full max-w-sm sm:max-w-md animate-in slide-in-from-top-6 fade-in-5 duration-300 ${notification.type === 'success'
        ...
        >
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <p
                className={`font-semibold text-sm ${notification.type === 'success' ? 'text-emerald-900' : 'text-red-900'
                  }`}
              >
              ...
              {notification.description && (
                <p
                  className={`mt-1 text-sm ${notification.type === 'success' ? 'text-emerald-700' : 'text-red-700'
                    }`}
                >
                  {notification.description}
                </p>
