...
export function ResetPasswordForm() {
  ...
  // Real-time validity — drives the inline red helper text + red border.
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!token) {
      toast.error("Invalid reset link", { description: "Missing or invalid token in the link." });
      return;
    }
    ...
    try {
      await authService.resetPassword({ token, newPassword: trimmedPassword });
      ...
      const msg = (err as { response?: { data?: { message?: string } }; message?: string })
        ...
        ?? (err instanceof Error ? err.message : "Failed to reset password. The link may be invalid or expired.");
        ...
        <Card className="border-sky-100/70 dark:border-slate-800/70 bg-white/95 dark:bg-slate-900/85 backdrop-blur-md shadow-2xl rounded-3xl overflow-hidden">
          <CardHeader className="px-8 pt-8 pb-4 text-center">
            ...
            <CardDescription className="mt-2 text-base">
              Choose a new password for your eBoard account.
            </CardDescription>
          </CardHeader>
          ...
          <CardContent className="px-8 pb-8">
            {!token ? (
              <p className="text-sm text-red-600 dark:text-red-400 text-center">
                This reset link is missing a token. Please use the link sent to
                your email.
              </p>
            ) : (
